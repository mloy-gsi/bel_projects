                           ...
                           uint32_t actStatus)                // actual status of firmware
{
  ...
  volatile uint16_t* pScuBaseAddress = (volatile uint16_t*)fwlib_getSbMaster();
  if (pScuBaseAddress == (uint16_t*)ERROR_NOT_FOUND) {
    pp_printf("no scub_base found!\n");
  }
  unsigned int slot = 2;
/*
  uint16_t registerValue = pScuBaseAddress[BUS_SLAVE_OFFSET(slot)+BUS_SLAVE_ECHO_REGISTER];
  pp_printf("echo %u...\n", registerValue);
  pScuBaseAddress[BUS_SLAVE_OFFSET(slot)+BUS_SLAVE_ECHO_REGISTER] = registerValue + 1;
*/
  ...
  switch (receivedTag) {
    ...
    default:
      ...
      pp_printf("event %x %x %x %x\n", receivedTag, recGid, recEvtNo, recSid);
      ...
      /*
      // combine
      // -gateware counter group represented by tag
      // -threshold parameter data set index rrpresented by sequence id
      // Add event key, gateware command and trigger diob tag
      diobTagValue = receivedTag & 0xffff0000; // upper 16 bit are event key expected by receiving diob BLM gateware
      diobTagValue |= 0x2 << 12; // EVT CMD PREPARE as EV_CMD at bit pos 12
      diobTagValue |= (receivedTag & 0xf) << 8; // counter group at bit pos 8
      diobTagValue |= (recSid & 0xff); // Dataset at bit pos 0
      pp_printf("diob tag: %x\n", diobTagValue);


      volatile uint16_t* pScuBusMasterBase = 0x40000;



      // use those register to write tag to SCU bus.
      // Write low part first, writing to high part triggers!
      volatile uint16_t *pSw_Tag_Low_Adr = pScuBaseAddress+0x400020;
      volatile uint16_t *pSw_Tag_High_Adr = pScuBaseAddress+0x400024;
      uint16_t low = diobTagValue & 0xffff;
      uint16_t high = diobTagValue >> 16;

      *pSw_Tag_Low_Adr = low;
      *pSw_Tag_High_Adr = high;
      */
      ...
      // manual mode
      {
        ...
        uint16_t registerValue = recSid & 0xff; // Dataset at bit pos 0
        registerValue |= (receivedTag & 0xf) << 8; // counter group at bit pos 8
        registerValue |= 0x1000; // trigger

        pp_printf("writing %x to %x\n", registerValue, BUS_SLAVE_OFFSET(slot)+EVENT_THRESHOLD_RELOAD_REGISTER);

        pScuBaseAddress[BUS_SLAVE_OFFSET(slot)+EVENT_THRESHOLD_RELOAD_REGISTER] = registerValue;
        pScuBaseAddress[BUS_SLAVE_OFFSET(slot)+EVENT_THRESHOLD_RELOAD_REGISTER] = 0;
      }
      break;
  }
